...
class ParentList(Generic[T], abc.ABC):
    """АТД связного списка."""

    HEAD_OK = 0 # последняя head() отработала нормально
    HEAD_ERR = 1 # список пуст

    TAIL_OK = 0 # последняя tail() отработала нормально
    TAIL_ERR = 1 # список пуст

    RIGHT_OK = 0 # последняя right() отработала нормально
    RIGHT_ERR = 1 # правее нет элемента

    PUT_RIGHT_OK = 0 # последний вызов put_right() отработал нормально
    PUT_RIGHT_ERR = 1 # список пуст

    PUT_LEFT_OK = 0 # последний вызов put_left() отработал нормально
    PUT_LEFT_ERR = 1 # список пуст
...
class TwoWayList(ParentList[T]):
    """Двунаправленный связный список, расширяющий родительский АТД."""

    LEFT_OK = 0 # последняя left() отработала нормально
    LEFT_ERR = 1 # левее нет элемента

    # предусловие: Есть узел слева
    ...
    def get_left_status() -> int:
        """Получить статус выполнения команды left. Возвращает значение LEFT_*"""